...
function log(message) {
  process.stderr.write(`unifi-rmcp: ${message}\n`);
}
...
async function main() {
  if (process.env.UNIFI_RMCP_SKIP_DOWNLOAD === "1") {
    log("skipping binary download because UNIFI_RMCP_SKIP_DOWNLOAD=1");
    return;
  }
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "unifi-rmcp-install-"));
